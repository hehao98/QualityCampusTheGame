...
对于教授而言，为了实现简单起见，我们设计了不同级别的教授level，每次升级时

#### 5. 舆论系统

舆论系统也就是这个游戏的随机事件系统。我们希望在舆论系统中能够模拟大学校园中常见的各种突发事件，并且逼迫玩家对这些突发事件作出处理。可能的事件包括：
...
事件的效果有小到大、可能是正面的也可能是负面的。发树洞是一种比较随意和常见的事件，不需要玩家做出处理，可以理解为是后续严重事件发生的一种风向标，我们希望学生发树洞是一个动态的不断生成的过程。而其他事件相对比较严重，一般是偶然发生的，发生频率较低，并且发生时需要玩家做出决策。

事件可以被分成很多类，每一类事件组成一个事件列表，这个列表从前往后负面性减少，正面性增加。每一个事件列表上的事件都是同一类满意度在不同正负面和严重程度情况下的具体反映。例如，如果要模拟学生的学习满意度，可以针对满意度的高低设计一组正负面和严重程度各不同的事件，根据满意度数值的高低和变化来触发。类似以下五个事件 1-2-3-4-5 1是负面事件（学生跳楼），2是微负面事件(学生发树洞说自己菜），3是中立事件(学生在树洞上问题目），4是微正面事件（学生努力学习），5是正面事件(媒体报道）。
