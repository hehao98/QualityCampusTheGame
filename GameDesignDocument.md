...
为了实现这样的事件系统，对于每一条事件链，需要一个Opinion Index，反应了学生在这个事件链上积累的舆情变化。设在时间t上积累的某个指标为$n_t$，那么它在t时刻的舆情指数为

$$
OpinionIndex = \sum_{i=t-k}^{t}n_t
$$
...
##### 7.2 学业指标
学业指标是评价学校的主要指标之一。无论是学习相关的各类满意度还是生活相关的各类满意度都会影响学业指标。目前学业指标的值是各类满意度的加权平均，学习相关的满意度的权值是生活类满意度的4倍。下轮迭代已计划引入更复杂的学业指标修正。
